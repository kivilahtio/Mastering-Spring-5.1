...
public class SpringSecurityConfiguration extends WebSecurityConfigurerAdapter {
	...
	public void configureGlobal(AuthenticationManagerBuilder auth) throws Exception {
		auth.inMemoryAuthentication().withUser("user1").password("{noop}user1-password").roles("ADMIN", "ALPHA");
		auth.inMemoryAuthentication().withUser("user2").password("{noop}user2-password").roles("USER", "ALPHA");
	}
